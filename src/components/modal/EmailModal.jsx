...
const EmailModal = ({ thread, isOpen, setIsOpen }) => {
  ...
  const handleSendReply = async (data) => {
    ...
    try {
      ...
      if (response?.data) {
        ...
        if (updatedThread.messages && updatedThread.messages.length > 0) {
  ...
  // Helper functions from EmailDetail
    ...
    >
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-gray-200 bg-white sticky top-0 z-10 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-gray-900 truncate flex-1 mr-4">
